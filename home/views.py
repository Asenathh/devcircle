    ...
    if request.method == 'POST':
        ...
        if cart:
            ...
            if basket:
                ...
                basket.save()
                messages.success(request, format_html("one item added... <a href='http://54.161.119.115/cart'>view cart</a>"))
                return redirect('home')
            else:
                newitem = Cart()
...
def pay(request):
    if request.method == 'POST':
        ...
        cburl = 'http://54.161.119.115/callback' #payment confirmation page
        ...
        data = {'reference': ref, 'amount': int(total), 'email': user.email, 'callback_url': cburl, 'order_number': order_no, 'currency': 'NGN'} #this is a dictionary, it stores key and value pair
        ...
        try:
            ...
            transhack = json.loads(r.text)
            rdurl = transhack['data']['authorization_url']
